...
Removes an unmounted device from the file system.
Will likely need root privileges.

```shell
# Once drive unmounted
eject path/to/drive
```
...
The directory used as a mount point must exist.
Use the `df` command to list all drives visible to the system.
Will likely need root privileges.
...
Note the absence of the letter `n` in the `umount` command.
Great stuff.
Will likely need root privileges.
